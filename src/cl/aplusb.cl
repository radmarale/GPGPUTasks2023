...
__kernel void aplusb(__global float* as, __global float* bs, __global float* cs, unsigned int size) {
    ...
    cs[id] = as[id] + bs[id];
    // Узнать, какой workItem выполняется в этом потоке
    // поможет функция get_global_id
    ...
    // P.S. В общем случае количество элементов для сложения может
    // быть некратно размеру WorkGroup, тогда размер рабочего
    // пространства округлен вверх от числа элементов до
    // кратности на размер WorkGroup
    // и в таком случае, если сделать обращение к массиву просто
    // по индексу=get_global_id(0), будет undefined behaviour
    // (вплоть до повисания ОС)
    // поэтому нужно либо дополнить массив данных длиной до
    // кратности размеру рабочей группы,
    // либо сделать return в кернеле до обращения к данным в
    // тех WorkItems, где get_global_id(0) выходит за границы
